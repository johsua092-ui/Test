...
metode: Specifies the HTTP method, either "GET" or "POST".
endpoint: The API path, should start with /api/ (e.g., /api/ai/gemini, /api/d/tiktok, /api/canvas/welcomev1, /api/sticker/combot-search). Dynamic parameters are denoted with colons (e.g., /api/user/:id).
name: A human-readable name for the endpoint.
category: The category this endpoint belongs to (used for documentation and organization). Should match the directory name in router/.
description: A detailed description of what the endpoint does.
tags: An array of string tags for categorization/searching in documentation.
example: A string showing example query parameters for a GET request (e.g., "?text=Hello&param=value").
parameters: An array defining query/path parameters for GET requests, following OpenAPI/Swagger specification.
requestBody: An object defining the structure of the JSON body for POST requests, following OpenAPI/Swagger specification.
isPublic: Boolean indicating if the endpoint is publicly accessible without an API key. Defaults to false if not specified.
isPremium: Boolean indicating if an API key is required to access this endpoint. Checked by the server's dynamic route handler.
isMaintenance: Boolean indicating if the endpoint is temporarily disabled.
run: An asynchronous function that contains the core logic for the endpoint.
...
Router baru sih terus backend nya di firebase, nah nanti juga gw mau ada apikey scrape Claude/glm gitu tapi ini kita buat yang gw bilg dulu Claude,glm,gpt,qwen,  nah nanti ini kita buat scrape nya itu free ga butuh apikey lagi buat akses model itu, tapi user harus generate apikey sendiri terus milih, nah waktu milih ini itu nanti di dashboard user 0 token palingan kita kasih 5$, sisah nya beli, nah terus misal si user beli token ke gw kira² 20rb nah nanti dapet tuh 1m token, buat nambahin token gitu dari database aja
